...
Publisher::Publisher(Wireless w, const char* host, const char* user, const char* pass, const char* pub_topic)
: m_user(user), m_pass(pass), m_wireless(w), m_client(w.client()), m_topic(pub_topic)
{
...
void Publisher::publish(const char* room, const Measure& m){
	...
	for( auto i = 0 ; i < m_buffer.size() ; i++ ){
		...
		m_client.publish(m_topic, payload.c_str());
	}
	m_buffer.clear();
}